                        ...
                        if (!$page_data || empty($page_data['entry_data']['ProfilePage'][0]['graphql']['user'])) {
                            ...
                            if ($user_data['is_private']) {
                                ...
                                if ($query_res['http_code'] != 200) {
                                    if (!empty($user_data['edge_owner_to_timeline_media']['edges'])) {
                                        $count = 12;
                                        $nodes = array();
...
function serve_tag_media_recent($tag) {
    ...
    if (!$raw_data) {
        ...
        if ($query_res['http_code'] != 200) {
            $page_res = client_request('get', '/explore/tags/' . $tag . '/?__a=1');
            $page_data = json_decode($page_res['body'], true);
            $hashtag_data = $page_data['graphql']['hashtag'];
...
function serve_location_media_recent($location_id) {
    ...
    if (!$raw_data) {
        ...
        if ($query_res['http_code'] != 200) {
            $page_res = client_request('get', '/explore/locations/' . $location_id . '/?__a=1');
            $page_data = json_decode($page_res['body'], true);
            $location_data = $page_data['graphql']['location'];

            if (!empty($location_data['edge_location_to_media']['edges'])) {
                $nodes = array();
