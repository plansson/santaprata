...
class Slideshow extends PHPFrodo
{
    ...
    public function welcome()
    {
        ...
        if ( $this->result() )
        {
          ...
          //  $this->preg( array('/\.jpg/','/\.png/'), array('',''),  'slide_url' );
            $this->preg( array( '/1/', '/2/','/3/','/4/','/5/' ), array( 'Slide Topo', 'Banner Produtos','Banner Topo','Banner Topo Meio','Banner Promo Topo' ), 'slide_local' );
            $this->preg( '/[0]/', '', 'slide_link' );
            // $this->preg( array( '/0/', '/1/' ), array( '', 'target="_blank' ), 'slide_target' );
    ...
    public function incluir()
    {
        if ( isset( $_FILES['filedata'] ) && strlen( $_FILES['filedata']['name'] ) >= 1 )
        {
            ...
            $this->slide_link = preg_replace('#^https?://#', '', rtrim($this->slide_link,'/'));
            ...
            $f = array( 'slide_title', 'slide_desc', 'slide_url', 'slide_link', 'slide_target', 'slide_local' );
            $v = array( "$this->slide_title", "$this->slide_desc", "$this->slide_url", "$this->slide_link", "$this->slide_target", "$this->slide_local" );
    ...
    public function atualizar()
    {
        ...
        // $this->slide_desc = $this->mytrim( $_POST['slide_desc'] ) ;
        $this->slide_link = $this->mytrim( $_POST['slide_link'] );
        $this->slide_target = $this->mytrim( $_POST['slide_target'] );
        $this->slide_local = $_POST['slide_local'];
        if ( $this->slide_link == "" || strlen( $this->slide_link ) <= 2 )
        {
        ...
        if ( isset( $_FILES['filedata'] ) && strlen( $_FILES['filedata']['name'] ) >= 1 )
        {
            ...
            {
                $this->removeAtual();
                $f = array( 'slide_url', 'slide_link','slide_target','slide_local' );
                $v = array( "$this->slide_url", "$this->slide_link", "$this->slide_target", "$this->slide_local" );
            }
        }
        else
        {
            $f = array( 'slide_link','slide_target','slide_local' );
            $v = array( "$this->slide_link","$this->slide_target","$this->slide_local" );
        }
        $this->update( 'slide' )->set( $f, $v )->where( "slide_id = $this->slide_id" )->execute();
       // print_r($this->slide_target);exit;
       $this->redirect( "$this->baseUri/admin/slideshow/process-ok/" );
    }
